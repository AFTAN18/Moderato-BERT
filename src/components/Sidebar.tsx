...
const navItems: { id: PageId; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  { id: 'model', label: 'BERT Performance', icon: BrainCircuit },
...
export default function Sidebar({ activeTab, setActiveTab, onSignOut }: SidebarProps) {
  const storedUser = (() => {
    try { return JSON.parse(localStorage.getItem('moderato_user') || '{}'); } catch { return {}; }
  })();
    ...
    <aside className="w-64 border-r border-brand-border bg-brand-surface flex flex-col h-screen fixed left-0 top-0 z-30 sidebar-desktop">
      ...
      <div className="p-6 flex items-center gap-3 border-b border-brand-border">
        ...
        <div className="flex flex-col">
          <span className="font-bold text-white tracking-tight text-sm leading-tight">Moderato</span>
          <span className="text-[10px] text-slate-600 font-mono">BERT-MOD v2.4</span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            <item.icon className={cn(
              ...
              activeTab === item.id ? "text-blue-400" : "text-slate-600 group-hover:text-slate-400"
            )} />
            {item.label}
          </button>
        ))}
        ...
        <div className="space-y-2.5 px-3">
          {[
            { name: 'BERT Inference', status: 'Online' },
      ...
      <div className="p-3 mt-auto border-t border-brand-border">
        <div className="flex items-center gap-3 p-2.5 bg-brand-card rounded-lg border border-brand-border mb-3">
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="text-[10px] text-slate-600 truncate">{storedUser.email || 'admin@moderato.ai'}</span>
          </div>
        </div>
        <button
          onClick={onSignOut}
